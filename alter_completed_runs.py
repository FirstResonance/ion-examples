...
def create_run_step(api: Api, run_id: int, title: str, content: str = None) -> dict:
    ...
    create_step = {
        ...
        "variables": {"input": {"title": title, "runId": run_id}},
    }
    run_step = api.request(create_step)
    return run_step["data"]["createRunStep"]["step"]


def create_run_step_field(
    api: Api, run_step_id: int, name: str, type: str, required: bool = False
) -> dict:
    """
...
def update_run_step_status(api: Api, run_step: dict, status: str) -> dict:
    ...
    update_run_step = {
        ...
        "variables": {
            "input": {"id": run_step["id"], "status": status, "etag": run_step["_etag"]}
        },
    }
...
def update_runs_with_new_info(api: Api, procedure_id: int) -> None:
    ...
    for run_id in runs:
        ...
        run_step_field = create_run_step_field(
            api=api,
        ...
        run_step = update_run_step_status(
            api=api, run_step=run_step, status="IN_PROGRESS"
        )
        # Fill the value for the newly created field
        run_step_field = update_run_step_field_value(
            api=api, run_step_field=run_step_field, value="value"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "procedure_id", type=int, help="ID of procedure that runs were created from."
    )
