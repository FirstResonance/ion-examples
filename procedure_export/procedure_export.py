...
def get_procedure_data(api: Api, procedure_id: int):
    ...
    """
    request_body = {
        'query': queries.GET_PROCEDURE,
        'variables': {'id': procedure_id}
    }
...
def add_asset(api: Api, asset: dict, step: dict):
    ...
    file_object = file_attachment_helper.download_file_attachment(asset["downloadUrl"], asset["filename"])
    new_file_attachment = create_ion_file_attachment(api, file_name, step["entityId"])
    file_attachment_helper.upload_file_attachment(file_object, file_name, new_file_attachment)
...
def update_step_slate_content(api: Api, etag: str, step_id: str, new_slate_content: str):
    request_body = {
        ...
        'variables': {
            'input': {
                "id": step_id,
                "etag": etag,
                "slateContent": new_slate_content
            }
        }
    }
    return api.request(request_body)
...
def add_steps(api: Api, source_procedure_data: dict, procedure_id: int, source_api: Api):
    for step in source_procedure_data['steps']:
        ...
        if slate_content:
            ...
            for match in re.finditer(expression, json.dumps(slate_content)):
                existing_file_attachment = get_file_attachment_info(source_api, match.groupdict()['id'])
                new_file_attachment = add_asset(api, asset, new_step)
                new_slate_content = json.dumps(slate_content).replace(match.group(), f"reference\": {new_file_attachment['fileAttachment']['id']}, ")
                update_step_slate_content(api, new_step["_etag"], new_step["id"], json.loads(new_slate_content))

def create_procedure_from_source_data(api: Api, source_procedure_data: dict, new_title: str, source_api: Api):
    """
...
if __name__ == '__main__':
    ...
    source_api = Api(client_id=source_client_id, client_secret=source_client_secret, auth_server=source_auth_server, api_uri=source_api_uri)
    target_api = Api(client_id=target_client_id, client_secret=target_client_secret, auth_server=target_auth_server, api_uri=target_api_uri)
