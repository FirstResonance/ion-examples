...
def update_ordered_status_to_draft(po_id, po_etag, api):
    request_body = {
        ...
        "variables": {"input": {"id": po_id, "etag": po_etag, "status": "DRAFT"}},
    }
...
def build_list_aboms_items(purchase_order_lines):
    for purchase_order_line in purchase_order_lines["purchaseOrderLines"]["edges"]:
        if "partInventories" in purchase_order_line["node"]:
            if (
                any(
                    part_inventory["installed"]
                    for part_inventory in purchase_order_line["node"]["partInventories"]
                )
                or any(
                    part_inventory["kitted"]
                    for part_inventory in purchase_order_line["node"]["partInventories"]
                )
                or any(
                    part_inventory["received"]
                    for part_inventory in purchase_order_line["node"]["partInventories"]
                )
                or any(
                    abom_child.get("partInventoryId") is not None
                    for part_inventory in purchase_order_line["node"]["partInventories"]
                    for abom_child in part_inventory.get("abomChildren", [])
                )
            ):
...
def delete_purchases(purchases, api):
    for purchase in purchases["purchaseOrders"]["edges"]:
        ...
        if (
            purchase_id in PURCHASES_TO_SKIP
            or purchase["node"]["approvals"]
            or purchase["node"]["fees"]
            or purchase["node"]["approvalRequests"]
        ):
