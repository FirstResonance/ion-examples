...
def get_attribute_files(
    api: Api, supplier_name: Optional[str] = None, page_size: int = PAGE_SIZE
) -> list[dict]:
...
def process_suppliers(
    ...
    for edge in nodes:
        ...
        if not attributes:
            logger.debug(
                f"No matching FILE_ATTACHMENT attributes found for '{supplier_name}'"
            )
            continue
        ...
        for attr in attributes:
            ...
            if not url:
                logger.warning(
                    f"Supplier '{supplier_name}' has empty URL for attribute '{key}', skipping."
                )
                continue

            if not is_valid_url(url):
                logger.warning(
                    f"Supplier '{supplier_name}' has invalid URL for attribute '{key}': {url}"
                )
                continue
            ...
            try:
                ...
                safe_supplier_name = (
                    re.sub(r"[^\w\s-]", "", supplier_name).strip().replace(" ", "_")
                )
                safe_key = re.sub(r"[^\w\s-]", "", key).strip().replace(" ", "_")

                # Create filename: supplier_name-attribute_key.extension
                filename = os.path.join(
                    output_dir, f"{safe_supplier_name}-{safe_key}{ext}"
                )

                # Check if file exists and handle accordingly
                if os.path.exists(filename) and not force_overwrite:
                    logger.warning(
                        f"File '{filename}' already exists, skipping. Use --force to overwrite."
                    )
                    continue

                logger.info(
                    f"Downloading '{key}' for supplier '{supplier_name}' to '{filename}'..."
                )
                ...
                logger.error(
                    f"Failed to download file for supplier '{supplier_name}', attribute '{key}': {e}"
                )
            except Exception as e:
                logger.error(
                    f"Error processing attribute '{key}' for supplier '{supplier_name}': {e}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--supplier-name", help="Supplier name (omit to download for all suppliers)"
    )
    parser.add_argument(
        "--attribute-names",
        required=True,
    ...
    parser.add_argument(
        "--force", action="store_true", help="Overwrite existing files without warning"
    )
    parser.add_argument(
        "--timeout",
        type=int,
